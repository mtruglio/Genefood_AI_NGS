...
def assemble_report(analysis_type, patient_id, raw_results, reports, scores_peso, scores_t2d, \
    ...
    if analysis_type in ['Base', 'Plus', 'Vita', "Sport", "Ageing", "Mamma"]:
        ...
        diete_for_AI.append("Predisposizione al Diabete di tipo 2 ({0}), Predisposizione alle malattie cardiovascolari ({1}), Predisposizione all'aumento di peso ({2}).".format(val_t2d.title().replace(' ',''), val_cardio.title().replace(' ',''), val_peso.title().replace(' ','')))
